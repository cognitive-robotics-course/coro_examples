/*******************************************************************************************************************
 *   Example camera calibration accounting for robot errors program for a LynxMotion AL5D robot arm
 *   --------------------------------------------------------------------
 *
 *   Implementation file
 *
...
{
    spawn_model(sdf_filename, CHECKERBOARD_MODEL_NAME, x, y, z, pitch, yaw, roll);
}
...
{
    delete_model(CHECKERBOARD_MODEL_NAME);
}
...
{
    ros::NodeHandle nh;
    ros::service::waitForService("/lynxmotion_al5d/reset");
    ros::ServiceClient client = nh.serviceClient<std_srvs::Empty>("/lynxmotion_al5d/reset");
    std_srvs::Empty srv;
    client.call(srv);
    wait(2000);
}
...
void moveToCoordinates(float ctrl_point_x, float ctrl_point_y, float ctrl_point_z, float object_phi)
{
    ...
    Frame T6;
    Frame ctrl_point;
    Frame ctrl_point_grasp;
    Frame ctrl_point_approach;
    ...
    ctrl_point          = trans(ctrl_point_x, ctrl_point_y, ctrl_point_z) * rotz(object_phi);      // ctrl_point pose
    ctrl_point_grasp    = trans(0, 0, 0) * roty(180);     // ctrl_point grasp frame w.r.t. both ctrl_point and destination frames
    ctrl_point_approach = trans(0, 0, -initial_approach_distance);                                      // frame defined w.r.t. grasp frame
    ...
    printf("Moving to ctrl_point approach pose\n");

    T6 = inv(Z) * ctrl_point * ctrl_point_grasp * ctrl_point_approach * inv(E);
    ...
    if (continuous_path) {
        ...
        while (approach_distance >= 0) {
            ...
            ctrl_point_approach   = trans(0, 0, -approach_distance);
    ...
    printf("Moving to ctrl_point pose\n");

    T6 = inv(Z) * ctrl_point * ctrl_point_grasp * inv(E);
...
void writeWorldCoordinatesToFile(FILE *fp_world_points, float topleftX, float topleftY, Size2f boxsize, Size size) {
    ...
    for (int j = 0; j < npointsY; j++) {
        for (int i = 0; i < npointsX; i++) {
            // printf("%3.4f %3.4f 0.0\n", topleftX + boxsize.width * i, topleftY + boxsize.height * j);
            fprintf(fp_world_points, "%3.4f %3.4f 0.0\n", topleftX + boxsize.width * i, topleftY + boxsize.height * j);
        }
    }
